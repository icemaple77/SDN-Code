...
def myNetwork():
    ...
    for s in range(i):
     ...
     c += 1
     if (c == 5): # the incriment
        switcheslayer2.append('ss%d' % (x+1))
        x += 1
        c = 0
        if (x == 5): # the incriment
          switcheslayer2.append('ss%d' % (y+1))
          y += 1
          x = 0
    ...
    i=n=0
    for s in switcheslayer2:
     net.addLink(s,switcheslayer3[i])
     info("(" + s + "," + switcheslayer3[i] + ") ")
     n += 1
     if (n == 10): #the amount of split of each switch can be done here n== 10 means use another swicth after the prev one has linked to 10 hosts
      i += 1
      n=0
    ...
    c=0
    for i in switcheslayer3:
     if ((c+1) < len(switcheslayer3)):
      net.addLink(i, switcheslayer3[c+1])
      c += 1
